...
install_iran_multi() {
  ...
  local iface wan mtu lp n
  iface="" # per-instance override not needed
  ...
  for ((i=1;i<=n;i++)); do
    bold "— سرور خارج #$i —"
    local rp vni port laddr raddr name ifc
    ...
    raddr="$(ask 'آدرس خصوصی سرور خارج /30 (فقط برای یادداشت)' "${laddr%/*/}10.8.$((80+i)).2/30")"
...
add_peer_iran() {
  ...
  while ! valid_cidr30 "$laddr"; do laddr="$(ask 'CIDR /30 نامعتبر؛ نمونه 10.8.99.1/30')"; end
